...
const PAGINATION_GUIDANCE_TIMEOUT_MS = 1500;
    ...
    switch (action) {
      case 'screenshot': {
        ...
        if (screenshot.status === 'success') {
          ...
          const paginationGuidance = context && !hasBudget(context, PAGINATION_GUIDANCE_TIMEOUT_MS)
            ? null
            : await withTimeout(
                getPaginationGuidance(page, tabId),
                PAGINATION_GUIDANCE_TIMEOUT_MS,
                'pagination-guidance',
                context
              ).catch(() => null);
